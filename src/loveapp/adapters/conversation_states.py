...
class InMemoryConversationFlowStateStore:
    def __init__(self) -> None:
        self._states: dict[tuple[str, str, str], ConversationFlowState] = {}
...
class SQLiteConversationFlowStateStore:
    """Small persistent state for pending tasks and clarification attempts."""
        ...
        *,
        ...
        async with aiosqlite.connect(self._database_path) as connection:
            ...
            cursor = await connection.execute(
                """
                SELECT state_json FROM conversation_flow_states
            ...
            await cursor.close()
        return ConversationFlowState.model_validate_json(row["state_json"]) if row else None
