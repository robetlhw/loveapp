"""Run the deterministic Memory Lifecycle V1 baseline and diagnostics."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Memory Lifecycle V1")
    parser.add_argument(
        "--dataset",
        type=Path,
        default=Path("evals/memory/lifecycle_v1.jsonl"),
    )
    parser.add_argument("--case", help="Evaluate one case, e.g. LIFE-023")
    ...
    parser.add_argument("--contract-status", choices=("EXACT", "POLICY_REVIEW"))
    ...
    report = evaluate_memory_lifecycle_v1(
        args.dataset,
        case_id=args.case,
    ...
    json_out = args.json_out or Path(".data/evals/memory_lifecycle_v1_baseline.json")
    md_out = args.md_out or Path("MEMORY_LIFECYCLE_V1_BASELINE_REPORT.md")
    policy_out = args.policy_review or Path("MEMORY_LIFECYCLE_POLICY_REVIEW.md")
    for path in (json_out, md_out, policy_out):
        path.parent.mkdir(parents=True, exist_ok=True)
    json_out.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    md_out.write_text(render_memory_lifecycle_v1_report(report), encoding="utf-8")
    policy_out.write_text(render_memory_lifecycle_policy_review(report), encoding="utf-8")

    filtered_run = any((args.case, args.slice_name, args.operation, args.contract_status))
    if not args.skip_integration and not filtered_run:
        integration = asyncio.run(evaluate_memory_lifecycle_integration(args.dataset))
        ...
        integration_md.write_text(
            render_memory_lifecycle_integration_diagnostic(integration),
            encoding="utf-8",
        )
